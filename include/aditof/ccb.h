...
const struct CAL_BLOCK_INFO_V1* ccb_get_cal_block_info(const ccb_data_t* const ccb_data, block_t block_id,
                                                       const size_t index);
const struct CAL_ADDRVAL_REG_BLOCK_V1* ccb_get_cal_block_addrval_reglist(const ccb_data_t* const ccb_data,
                                                                         const size_t index);
const struct CAL_FOI_MASK_BLOCK_V0_V1INFO* ccb_get_cal_block_foi_mask(const ccb_data_t* const ccb_data,
                                                                      const size_t index);
const struct CAL_GEOMETRIC_BLOCK_V3* ccb_get_cal_block_geometric(const ccb_data_t* const ccb_data, const size_t index);
const struct CAL_ILLUM_PROFILE_BLOCK_V2* ccb_get_cal_block_illum_profile(const ccb_data_t* const ccb_data,
                                                                         const size_t index);
...
const struct CAL_SPATIAL_TEMP_CORR_BLOCK* ccb_get_cal_block_spatial_tempcor(const ccb_data_t* const ccb_data,
                                                                            const size_t index);
const struct CAL_TEMP_CORR_BLOCK_V0_V1INFO* ccb_get_cal_block_tempcorrection(const ccb_data_t* const ccb_data,
                                                                             const size_t index);
const struct CAL_RELATIVE_ILLUM_BLOCK* ccb_get_cal_block_relative_illum(const ccb_data_t* const ccb_data,
                                                                        const size_t index);
const struct CAL_GAIN_CORRECTION_BLOCK* ccb_get_cal_block_gaincorrection(const ccb_data_t* const ccb_data,
                                                                         const size_t index);
...
const ccb_index_t* ccb_build_cal_block_index(const ccb_data_t* const ccb_data, unsigned char* buffer,
